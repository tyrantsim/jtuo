...
public class FieldSimulator {
    ...
    /**
     * Starting point of FieldSimulator
     */
    ...
    /*
     * Pre-phases setup [START]
     */

    /**
     * Play dominions and fortresses before turn 1
     */
    ...
    /**
     * Start Phase: Reduce timers and set indexes
     */
    ...
    /**
     * Play Card Phase: Play a card, resolve on-play skills, resolve passive BGEs
     */
    private static void simulatePlayCardPhase(Field field, Card playedCard) {
        ...
        debug(2, "Play card: %s {%s}", playedStatus.description(), Cards.cardDescription(playedCard));
        resolveSkill(field); // resolve postponed skills recursively
        ...
        for (CardStatus status : field.getTap().getAssaults()) {
            ...
            if (field.hasBGEffect(field.getTapi(), PassiveBGE.COLDSLEEP)) {
                int bgeValue = (int) (((float) status.getProtectedByStasis() + 1) / 2);
    ...
    /**
     * Evaluate structures: check if active and evaluate skills
     */
    private static void evaluateStructures(Field field) {
        field.setCurrentPhase(FieldPhase.STRUCTURES_PHASE);
        for (field.setCurrentCI(0); !field.isEnd() && (field.getCurrentCI() < field.getTap().getStructures().size()); field.incrementCI()) {
            CardStatus currentStatus = field.getTap().getStructures().get(field.getCurrentCI());
    ...
    /**
     * Evaluate assaults: skills and direct attack
     */
    ...
    private static Boolean evaluateSkills(Field field, CardStatus status, List<SkillSpec> skills, Boolean attacked) {
        ...
        for (int actionIndex = 0; actionIndex < numActions; actionIndex++) {
            ...
            if (numActions > 1)
                debug(3, "%s activating flurry", status.description());
    ...
    /**
     * End Phase: Reset status and remove dead cards
     */
    ...
    private static void resolveSkill(Field field) {
        ...
        while (!field.getSkillQueue().isEmpty()) {
            ...
            debug(2, "Resolving skill: %s >> %s", status.description(), ss.description());
    ...
    private static void performSkill(Skill skillId, Field field, CardStatus src, CardStatus dst, SkillSpec s) {
        switch (skillId) {
            ...
            case STRIKE:
                ...
                int strikeDmg = safeMinus(s.getX() + dst.getEnfeebled(),
                        src.isOverloaded() ? 0 : dst.protectedValue());
                removeHP(field, dst, strikeDmg);
                break;
    ...
    private static void performTargettedAlliedFast(Skill skillId, Field field, CardStatus src, SkillSpec s) {
        ...
        int numInhibited = 0;
        for (CardStatus dst: (ArrayList<CardStatus>) field.selectionArray.clone()) {
    ...
    private static int performAttack(Field field, CardType enemyCardType, CardStatus attStatus, CardStatus defStatus) {
        ...
        if (defStatus.hasSkill(Skill.COUNTER)) {
            ...
            int counterDmg = safeMinus(defStatus.skill(Skill.COUNTER) + attStatus.getEnfeebled(),
                    attStatus.getProtectedBy());
    ...
    /*
     * Field & CardStatus modifications [START]
     */
    ...
    private static void removeHP(Field field, CardStatus status, int dmg) {
        ...
        if (field.getCurrentPhase().ordinal() < FieldPhase.END_PHASE.ordinal() && status.hasSkill(Skill.BARRIER)) {
            ...
            debug(2, "%s damaged %d times", status.description(), field.getDamagedUnitsToTimes(status));
        }
        if (status.getHP() == 0) {
